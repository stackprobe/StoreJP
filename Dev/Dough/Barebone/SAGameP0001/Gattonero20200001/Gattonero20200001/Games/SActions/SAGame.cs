		...
		public void Run(string fieldName)
		{
			...
			for (this.Frame = 0; ; this.Frame++)
			{
				...
				// プレイヤー入力
				{
					...
					if (Inputs.C.GetInput() == 1)
					{
						weaponChange = 1;
					}
					if (Inputs.D.GetInput() == 1)
					{
						weaponChange = -1;
					}
					...
					if (weaponChange != 0)
					{
						GameStatus.I.EquippedWeapon += weaponChange;

						const int WEAPON_NUM = 4;

						GameStatus.I.EquippedWeapon += WEAPON_NUM;
						GameStatus.I.EquippedWeapon %= WEAPON_NUM;
					}
				}
			...
			endOfInvincible:
				...
				// -- プレイヤーのダメージ中・無敵時間中など、当たり判定無しの場合は Crash.CreateNone() をセットすること。
				{
					...
					{
						this.Player.Crash = Crash.CreateRect(D4Rect.XYWH(this.Player.X, this.Player.Y + 10.0, 14.0, 38.0));
					}
					else if (1 <= this.Player.MoveFrame)
					{
						this.Player.Crash = Crash.CreateRect(D4Rect.XYWH(this.Player.X, this.Player.Y + 2.0, 14.0, 54.0));
					}
					else
					{
						this.Player.Crash = Crash.CreateRect(D4Rect.XYWH(this.Player.X, this.Player.Y + 0.0, 14.0, 58.0));
					}
				}
...
";
		...
		{
			DD.SetPrint(0, 0, 16);
			DD.SetPrintBorder(new I3Color(64, 128, 64), 1);
			DD.PrintLine("HP : " + GameStatus.I.HP);
			DD.PrintLine("EW : " + GameStatus.I.EquippedWeapon);
		}
