...
namespace Charlotte.Games.TActions.Enemies
{
	...
	public abstract class TAEnemy
	{
		...
		// 敵：
		// -- new TAEnemy(x, y, hp, attackPoint, false)
		// 敵弾：
		// -- new TAEnemy(x, y, 0, attackPoint, 自機に当たると消滅する)
		// アイテム：
		// -- new TAEnemy(x, y, 0, 0, false) -- 当たり判定を設置しない。
		// イベント：
		// -- new TAEnemy(x, y, 0, 0, false) -- 描画しない。当たり判定も設置しない。

		public TAEnemy(double x, double y, int hp, int attackPoint, bool 自機に当たると消滅する)
		{
			this.X = x;
			this.Y = y;
			this.HP = hp;
		...
		public void Damaged(TAShot shot, int damagePoint)
		{
		...
		/// <param name="shot">この敵が被弾したプレイヤーの弾</param>
		/// <param name="damagePoint">削られた体力</param>
		protected virtual void P_Damaged(TAShot shot, int damagePoint)
		{
		...
		{
			TAEnemyCommon.Killed(this, destroyed);
		}
	}
}
