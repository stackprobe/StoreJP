...
namespace Charlotte.Games
{
	public static class TitleMenu
	{
		...
		public static void Run()
		{
			...
			Musics.CrazyHalloweenNight.Play();
			...
			DrawWall = () =>
			{
				...
				DD.SetZoom(z);
				DD.Draw(Pictures.TitleWall, new I2Point(GameConfig.ScreenSize.W / 2, GameConfig.ScreenSize.H / 2).ToD2Point());
			};
		...
		{
			string serializedString = ContinueMenu.Run();

			if (serializedString != "")
			{
				SoundEffects.Load.Play();
				GameStatus.I.Deserialize(serializedString);
				Leave();

				using (new SAGameMaster())
				{
					SAGameMaster.I.Run(GameStatus.I.LastSavedFieldName);
				}
				Enter();
			}
		}
