		...
		protected override IEnumerable<bool> E_Draw()
		{
			...
			for (int frame = 0; ; frame++)
			{
				...
				// 跳ね返り
				{
					...
					if (SAGame.I.Field.IsWall(SACommon.ToTablePoint(new D2Point(this.X - R, this.Y))))
					{
					...
					if (SAGame.I.Field.IsWall(SACommon.ToTablePoint(new D2Point(this.X, this.Y - R))) && yAdd < 0.0)
					{
						yAdd *= -0.98;
						bounced = true;
					}
					if (SAGame.I.Field.IsWall(SACommon.ToTablePoint(new D2Point(this.X, this.Y + R))) && 0.0 < yAdd)
					{
						yAdd *= -0.98;
						bounced = true;

						while (
							!SAGame.I.Field.IsWall(SACommon.ToTablePoint(new D2Point(this.X, this.Y))) &&
							SAGame.I.Field.IsWall(SACommon.ToTablePoint(new D2Point(this.X, this.Y + R)))
							)
							this.Y -= 1.0;
					}
