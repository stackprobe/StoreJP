		...
		public void Run(SAField field)
		{
			...
			endOfInvincible:
				...
				// -- プレイヤーのダメージ中・無敵時間中など、当たり判定無しの場合は Crash.CreateNone() をセットすること。
				{
					...
					{
						this.Player.Crash = Crash.CreateRect(D4Rect.XYWH(this.Player.X, this.Player.Y + 7.0, 14.0, 30.0));
					}
					else
					{
						this.Player.Crash = Crash.CreateRect(D4Rect.XYWH(this.Player.X, this.Player.Y + 5.0, 14.0, 34.0));
					}
				}
